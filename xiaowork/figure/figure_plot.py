...
class Figure_OEE(Figure_Origin):
    ...
    def plot(self, *args, **kwargs):
        def plot_oee():
            ...
            type1 = axes.plot(X, Y, label="OEE变化对比", c='g', lw=2, marker='s',
                              mec='r', mfc='r', ms='5')

            # 加标题
            axes.set_title("侧板效能分时图", fontsize=20)
            ...
            for a, b in zip(X, Y):
                axes.text(a + 0.1, b + 0.01, '%1.f%%' % b, ha='center', va='bottom',
                          color='purple', fontsize=10)  # 25
