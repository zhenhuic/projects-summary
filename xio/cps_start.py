                ...
                with f:
                    ...
                    if self.label.text() == "生产流程模型(厚板线)":
                        ...
                        for i in range(len(data)):
                            if self.label_hp1.text() in data[i]:
                                hp1_time.append(float(data[i].split(" ")[1][:-1]))
                                hp1_fail.append(data[i].split(" ")[2])
                            if self.label_hp2.text() in data[i]:
                                hp2_time.append(float(data[i].split(" ")[1][:-1]))
                                hp2_fail.append(data[i].split(" ")[2])
                            if self.label_hp3.text() in data[i]:
                                hp3_time.append(float(data[i].split(" ")[1][:-1]))
                                hp3_fail.append(data[i].split(" ")[2])
                            if self.label_hp4.text() in data[i]:
                                hp4_time.append(float(data[i].split(" ")[1][:-1]))
                                hp4_fail.append(data[i].split(" ")[2])
                            if self.label_hp5.text() in data[i]:
                                hp5_time.append(float(data[i].split(" ")[1][:-1]))
                                hp5_fail.append(data[i].split(" ")[2])
                            if self.label_hp6.text() in data[i]:
                                hp6_time.append(float(data[i].split(" ")[1][:-1]))
                                hp6_fail.append(data[i].split(" ")[2])
                            if self.label_hp7.text() in data[i]:
                                hp7_time.append(float(data[i].split(" ")[1][:-1]))
                                hp7_fail.append(data[i].split(" ")[2])
                            if self.label_hp8.text() in data[i]:
                                hp8_time.append(float(data[i].split(" ")[1][:-1]))
                                hp8_fail.append(data[i].split(" ")[2])
                            if self.label_hp9.text() in data[i]:
                                hp9_time.append(float(data[i].split(" ")[1][:-1]))
                                hp9_fail.append(data[i].split(" ")[2])
                            if self.label_hp10.text() in data[i]:
                                hp10_time.append(float(data[i].split(" ")[1][:-1]))
                                hp10_fail.append(data[i].split(" ")[2])
                        ...
                        self.textEdit.append(self.label_hp1.text() + self.label_ht1.text() + " 故障率：" + str(hp1_fail_count / len(hp1_fail)) + "\n")
                        self.textEdit.append(self.label_hp2.text() + self.label_ht2.text() + " 故障率：" + str(hp2_fail_count / len(hp2_fail)) + "\n")
                        self.textEdit.append(self.label_hp3.text() + self.label_ht3.text() + " 故障率：" + str(hp3_fail_count / len(hp3_fail)) + "\n")
                        self.textEdit.append(self.label_hp4.text() + self.label_ht4.text() + " 故障率：" + str(hp4_fail_count / len(hp4_fail)) + "\n")
                        self.textEdit.append(self.label_hp5.text() + self.label_ht5.text() + " 故障率：" + str(hp5_fail_count / len(hp5_fail)) + "\n")
                        self.textEdit.append(self.label_hp6.text() + self.label_ht6.text() + " 故障率：" + str(hp6_fail_count / len(hp6_fail)) + "\n")
                        self.textEdit.append(self.label_hp7.text() + self.label_ht7.text() + " 故障率：" + str(hp7_fail_count / len(hp7_fail)) + "\n")
                        self.textEdit.append(self.label_hp8.text() + self.label_ht8.text() + " 故障率：" + str(hp8_fail_count / len(hp8_fail)) + "\n")
                        self.textEdit.append(self.label_hp9.text() + self.label_ht9.text() + " 故障率：" + str(hp9_fail_count / len(hp9_fail)) + "\n")
                        self.textEdit.append(self.label_hp10.text() + self.label_ht10.text() + " 故障率：" + str(hp10_fail_count / len(hp10_fail)) + "\n")
                    ...
                    if self.label.text() == "生产流程模型(专机线)":
                        ...
                        zp6_fail_count = 0
                        for i in range(len(data)):
                            if self.label_zp1.text() in data[i]:
                                zp1_time.append(float(data[i].split(" ")[1][:-1]))
                                zp1_fail.append(data[i].split(" ")[2])
                            if self.label_zp2.text() in data[i]:
                                zp2_time.append(float(data[i].split(" ")[1][:-1]))
                                zp2_fail.append(data[i].split(" ")[2])
                            if self.label_zp3.text() in data[i]:
                                zp3_time.append(float(data[i].split(" ")[1][:-1]))
                                zp3_fail.append(data[i].split(" ")[2])
                            if self.label_zp4.text() in data[i]:
                                zp4_time.append(float(data[i].split(" ")[1][:-1]))
                                zp4_fail.append(data[i].split(" ")[2])
                            if self.label_zp5.text() in data[i]:
                                zp5_time.append(float(data[i].split(" ")[1][:-1]))
                                zp5_fail.append(data[i].split(" ")[2])
                            if self.label_zp6.text() in data[i]:
                                zp6_time.append(float(data[i].split(" ")[1][:-1]))
                                zp6_fail.append(data[i].split(" ")[2])
                    ...
                    if self.label.text() == "生产流程模型(薄板线)":
                        ...
                        bp5_fail_count = 0
                        for i in range(len(data)):
                            if self.label_bp1.text() in data[i]:
                                bp1_time.append(float(data[i].split(" ")[1][:-1]))
                                bp1_fail.append(data[i].split(" ")[2])
                            if self.label_bp2.text() in data[i]:
                                bp2_time.append(float(data[i].split(" ")[1][:-1]))
                                bp2_fail.append(data[i].split(" ")[2])
                            if self.label_bp3.text() in data[i]:
                                bp3_time.append(float(data[i].split(" ")[1][:-1]))
                                bp3_fail.append(data[i].split(" ")[2])
                            if self.label_bp4.text() in data[i]:
                                bp4_time.append(float(data[i].split(" ")[1][:-1]))
                                bp4_fail.append(data[i].split(" ")[2])
                            if self.label_bp5.text() in data[i]:
                                bp5_time.append(float(data[i].split(" ")[1][:-1]))
                                bp5_fail.append(data[i].split(" ")[2])
