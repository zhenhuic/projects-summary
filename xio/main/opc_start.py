...
class MainApp(QMainWindow, Ui_MainWindow):
    ...
    def export(self):
        if self.selected_flag is True:
            ...
            start = str(start_date).split('(')[1][:-5].replace(',', "/") + str(start_time).split('(')[1][11:].split(')')[0].replace(',', ':')+'0'
            end = str(end_date).split('(')[1][:-5].replace(',', "/") + str(end_time).split('(')[1][11:].split(')')[0].replace(',', ':')+'0'
    ...
    def plot(self):
        time.sleep(1)
        if self.selected_flag is True:
            self.flc.plotlinechart(self.comboBox_5.currentText(), self.comboBox_2.currentText(), self.comboBox_4.currentText())
            if self.comboBox_5.currentText() == "日平均(7日)":
                pix = QPixmap("1.png")
                self.label_34.setPixmap(pix)
            elif self.comboBox_5.currentText() == "周平均(30日)":
                pix = QPixmap("2.png")
                self.label_34.setPixmap(pix)
        else:
            return
