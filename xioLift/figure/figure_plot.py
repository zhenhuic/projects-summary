...
class Figure_171(Figure_Origin):
    '''这个类是绘制耗材图
    '''

    def __init__(self, parent=None, width=3.5, height=2.8, dpi=100):
        super(Figure_171, self).__init__()
    ...
    def plot(self, *args, **kwargs):
        def plot_mt():
            ...
            ax.set_title("171工作损失时间统计", fontsize=20)  # 30
            ax.set_xticklabels((u'机器工作', u'材料切换', u'工人拖料'), fontsize=15)
            ax.yaxis.grid(True)
            self.canvas.draw()
            plt.close()
