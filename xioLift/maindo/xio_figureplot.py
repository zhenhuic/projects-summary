...
from figure.figure_plot import Figure_Pie, Figure_171, Figure_Loss, Figure_OEE, Figure_Origin
...
class XioFigurePlot(QtGui.QWidget):
    ...
    def draw(self):
        ...
        def draw_171():  # 绘制耗材使用图
            f171 = Figure_171()
            hb171 = data_access.HB171()
            result171 = hb171.select_("select * from houban171 ORDER BY SJC DESC limit 1")
            f171.plot(*(result171[-1][1], result171[-1][2], result171[-1][3]))
            graphicscene_mt = QtGui.QGraphicsScene()
            graphicscene_mt.addWidget(f171.canvas)
